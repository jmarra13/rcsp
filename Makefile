# This Makefile is for the CSP extension to perl.
#
...
#     EXE_FILES => [q[csp]]
#     NAME => q[CSP]
#     PREREQ_PM => { Date::Calc=>q[0], IPC::Run=>q[0], Term::Prompt=>q[0.1] }
#     VERSION_FROM => q[CSP.pm]
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  CSP.pm $(INST_MAN3DIR)/CSP.$(MAN3EXT) 
...
$(INST_SCRIPT)/csp : csp $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/csp
	$(CP) csp $(INST_SCRIPT)/csp
	$(FIXIN) $(INST_SCRIPT)/csp
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/csp
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                CSP' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Term::Prompt:                  0.1' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,34,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Date-Calc" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="IPC-Run" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Term-Prompt" VERSION="0,1,0,0" />' >> $(DISTNAME).ppd
